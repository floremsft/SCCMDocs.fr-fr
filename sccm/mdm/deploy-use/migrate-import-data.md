...
- Il existe des profils qui dépendent d’autres objets. Si vous souhaitez importer un profil qui dépend d’un autre objet, comme un profil d’e-mail qui dépend d’un certificat, vous devez importer les deux objets en même temps.
...
- Vous devez exécuter l’outil à partir d’un ordinateur qui a accès au fournisseur SMS (pour collecter les données Configuration Manager) et à Internet (pour importer des objets dans Intune).
...
1. Accédez à la page [Intune Data Importer GitHub](https://go.microsoft.com/fwlink/?linkid=858194).
2. Cliquez sur **Cloner ou télécharger**, cliquez sur **Télécharger le ZIP**, puis enregistrez le fichier ZIP compressé. 
...
3. Indiquez les informations suivantes sur votre site et les objets sur le site à importer. 
...
5.  Cliquez sur **Suivant** pour passer en revue les objets collectés. Une page de sélection d’éléments est disponible pour chaque type d’objet collecté. 
    ...
    > [!Tip]     
    > Dans chaque page de sélection d’éléments, vous pouvez créer un filtre pour mieux trouver les objets à importer. Toutefois, notez les points suivants :
    > - Lorsque vous êtes dans une page de sélection d’éléments et que la vue est filtrée, les cases Sélectionner tout s’appliquent uniquement aux éléments affichés. Tous les objets masqués en raison d’un filtre ne sont pas inclus quand vous utilisez les cases à cocher.
    > - Les objets sont toujours regroupés sous leur élément parent, même lorsque vous les triez ou les filtrez.
...
8.  Dans chaque page de sélection d’éléments, sélectionnez les objets que vous voulez importer. Les colonnes suivantes sont listées :
    ...
    - **Remarques** : fournit des informations sur la raison pour laquelle un objet ne peut pas être importé.
...
### <a name="phase-3-import-selected-object-to-intune"></a>Phase 3 : Importer l’objet sélectionné dans Intune
Dans la phase 3, connectez-vous à Intune et importez les objets sélectionnés. 
10. Cliquez sur **Suivant**, puis sur **Se connecter à Intune** pour vous connecter au locataire Intune pour l’importation de données. Vous devez vous connecter avec un compte d’administrateur général ou d’administrateur Intune. Une fois connecté, le processus d’importation démarre.
