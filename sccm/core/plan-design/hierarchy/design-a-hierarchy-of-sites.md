...
-   Élimine le retard possible engendré par la réplication de base de données entre sites.  
...
-   Prend en charge jusqu’à 25 sites principaux, ce qui vous permet d’étendre l’échelle de votre hiérarchie.  

-   Ce choix est définitif. Vous ne pouvez pas détacher un site principal enfant pour en faire un site principal autonome. Ainsi, sauf si vous réinstallez vos sites, vous utiliserez toujours le site administration centrale.  
...
-   Quand vous configurez une hiérarchie comprenant plusieurs sites principaux, vous devez installer un site d’administration centrale qui doit être le premier site installé.  
...
-   Un site principal prend en charge uniquement des sites secondaires en tant que sites enfants, et peut prendre en charge plusieurs sites enfants secondaires.  

-   Les sites principaux sont chargés de traiter toutes les données clientes provenant de leurs clients attribués.  

-   Les sites principaux utilisent la réplication de base de données pour communiquer directement avec leur site d’administration centrale (ceci est configuré automatiquement lors de l’installation d’un nouveau site).  
...
-   Une installation de site secondaire est lancée à partir de la console Configuration Manager plutôt qu’en exécutant le programme d’installation de Configuration Manager directement sur un ordinateur.  
...
-   Les sites secondaires prennent en charge l’acheminement du contenu basé sur des fichiers vers d’autres sites secondaires qui ont un site principal parent commun.  
...
-   Le service de transfert intelligent en arrière-plan (BITS, Background Intelligent Transfer Service) ne fournit pas de contrôle de bande passante suffisant pour les besoins de votre réseau.  
...
-   Identifiez les [Features and capabilities of System Center Configuration Manager](../../../core/plan-design/changes/features-and-capabilities.md) (fonctionnalités et capacités de System Center Configuration Manager) que vous envisagez d’utiliser, les rôles de système de site ou l’infrastructure Windows nécessaires ainsi que les sites au niveau desquels dans une hiérarchie comportant plusieurs sites vous pouvez les déployer pour une utilisation optimale du réseau et des ressources serveur.  
...
-   Allez-vous [étendre le schéma Active Directory pour System Center Configuration Manager](../../../core/plan-design/network/extend-the-active-directory-schema.md) et configurer des sites pour la [publication de données de site pour System Center Configuration Manager](../../../core/servers/deploy/configure/publish-site-data.md) ?  

-   Pour gérer la bande passante réseau entre les sites d’une hiérarchie, consultez [Transfert de données entre sites dans System Center Configuration Manager](../../../core/servers/manage/data-transfers-between-sites.md)  
