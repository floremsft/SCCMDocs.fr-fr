---
title: "Créer des applications Windows Phone | System Center Configuration Manager"
...
## <a name="steps-to-deploy-the-latest-windows-phone-company-portal-app-with-supersedence"></a>Étapes de déploiement de la dernière version de l’application de portail d’entreprise Windows Phone  
...
|**Étape 4 :** ajouter l’application à l’Assistant Créer un abonnement Windows Intune.|Ajoutez la page Windows Phone 8 de l’application dans l’Assistant Créer un abonnement Microsoft Intune. Pour plus d’informations, consultez [Configurer la gestion des appareils Windows Phone et Windows 10 Mobile hybrides avec System Center Configuration Manager et Microsoft Intune](../../mdm/deploy-use/set-up-windows-phone-hybrid-enrollment.md).|  
...
|**Étape 6 :** créer un déploiement de l’application et sélectionner **Mettre automatiquement à niveau toutes les versions remplacées de cette application** dans la page **Paramètres de déploiement** de l’ **Assistant Déploiement logiciel**.|Créez un nouveau déploiement avec le remplacement à l'aide de l'application que vous avez créée avec la relation de remplacement.|  
|**Étape 7 (facultative) :** par défaut, les applications de remplacement s’installent sur les appareils au bout de sept jours. Pour déployer l'application de portail d'entreprise plus tôt sur les appareils inscrits auparavant, vous pouvez définir le paramètre **Planifier la réévaluation des déploiements** sur une valeur inférieure.<br /><br /> Si vous affectez à cette valeur une valeur inférieure à la valeur par défaut, cela peut affecter négativement les performances de votre réseau et des ordinateurs clients.|Aucune information supplémentaire.|  
