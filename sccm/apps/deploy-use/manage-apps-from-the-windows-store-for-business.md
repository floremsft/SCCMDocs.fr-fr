...
- Les PC doivent être joints à l’espace de travail Azure Active Directory et se trouver dans le même client AAD où vous avez enregistré le Microsoft Store pour Entreprises comme outil de gestion.
...
- Lorsque l’installation est appliquée par l’utilisateur qui installe l’application, par l’application qui atteint son échéance d’installation ou en cas de réévaluation post-installation des déploiements obligatoires :
    ...
    - L’état de l’application dans la console Configuration Manager indique un échec avec l’erreur « L’application Microsoft Store a été ouverte sur le PC client et attend que l’utilisateur termine l’installation. »
...
- Si vous avez déployé une stratégie de groupe qui désactive l’accès à la version grand public de Microsoft Store, les déploiements à partir du Microsoft Store pour Entreprises ne fonctionnent pas, même si le Microsoft Store pour Entreprises est activé.
...
4. À la fin de l’opération, le compte que vous avez configuré apparaît dans la liste **Microsoft Store pour Entreprises** de la console Configuration Manager.
...
6. Passez en revue la page **Informations**, puis effectuez les étapes et configurations supplémentaires indiquées. Ces configurations sont nécessaires pour utiliser le service avec Configuration Manager. Par exemple, pour configurer Microsoft Store pour Entreprises :
    ...
    - Dans la console Microsoft Store pour Entreprises, vous devez configurer Configuration Manager comme outil de gestion du Store, activer la prise en charge pour les applications sous licence en mode hors connexion, puis acheter au moins une application. 
...
Après le déploiement d’applications en ligne, notez que toute mise à jour de cette application proviendra directement du Microsoft Store.

Quand vous déployez des applications hors connexion sur des appareils Windows 10 avec le client Configuration Manager (en particulier dans les environnements multi-utilisateurs tels que des salles de classe), désactivez le Microsoft Store et/ou les mises à jour d’application par le biais du Microsoft Store (par exemple, à l’aide d’une [stratégie de groupe](https://docs.microsoft.com/en-us/windows/configuration/stop-employees-from-using-microsoft-store#a-href-idblock-store-group-policyablock-microsoft-store-using-group-policy)) pour que les utilisateurs ne puissent pas mettre à jour des applications externes aux déploiements Configuration Manager. Par ailleurs, une fois que l’administrateur du Microsoft Store pour Entreprises achète l’application (la rendant ainsi disponible hors connexion à des fins de synchronisation avec Configuration Manager), ne publiez pas l’application pour empêcher les utilisateurs de l’installer ou de la mettre à jour en ligne. De cette façon, tous les utilisateurs recevront uniquement les mises à jour de l’application par le biais de Configuration Manager. 
