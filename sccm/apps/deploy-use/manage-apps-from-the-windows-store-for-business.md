...
|Prise en charge des applications métier à partir du magasin|Oui|Oui|
...
Avant de déployer des applications Windows Store pour Entreprise sur des PC exécutant le client Configuration Manager complet, procédez comme suit :
...
- Lorsque l’installation est appliquée par l’utilisateur qui installe l’application ou que l’application atteint son échéance d’installation ou en cas de réévaluation post-installation pour les déploiements requis :
    - L’application sera mise en œuvre en lançant l’application Windows Store pour Entreprises. 
    - L’utilisateur final doit alors terminer l’installation à partir du magasin avant qu’elle soit réellement installée
    - L’état de l’application dans la console Configuration Manager indiquera un échec avec l’erreur « L'application Windows Store a été ouverte sur le PC client et attend que l'utilisateur termine l'installation. »
- Au prochain cycle d’évaluation de l’application :
    - Si l’application a été installée par l’utilisateur final à partir du magasin, l’application affichera l’état **Réussite**. 
    - Si l’utilisateur final n’a pas essayé d’installer l’application à partir du magasin :
        - Les déploiements requis tenteront de lancer le magasin et de remettre en œuvre l’installation de l’application.
        - Les déploiements disponibles ne seront pas remis en œuvre.
...
- Vous ne pouvez pas déployer des applications métier personnalisées à partir du Windows Store pour Entreprises
- Lorsque vous déployez des applications payantes à partir du magasin, les utilisateurs finaux seront invités à se connecter au magasin et à acheter eux-mêmes l’application.
- Si vous avez déployé une stratégie de groupe qui désactive l’accès à la version grand public de Windows Store, les déploiements à partir du Windows Store pour Entreprises ne fonctionneront pas, même si le Windows Store pour Entreprises est activé.
